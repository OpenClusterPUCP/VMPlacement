...
class VirtualMachine:
    ...
    def calcular_estadisticas_de_uso(self, profile: str) -> Dict[str, Tuple[float, float]]: # 3.4. Cálculo de Uso Promedio y Variabilidad
        """
        ...
        return {
            "cpu": (round(mu_cpu, 2), round(sigma_cpu, 2)), #EVALUAR REDONDEO
            "ram": (round(mu_ram, 2), round(sigma_ram, 2)),
            "disk": (round(mu_disk, 2), round(sigma_disk, 2))
        }
...
class Slice:
    ...
    def calcular_capacidad_efectica_slice(self, k: float = 2.0) -> Dict[str, float]: # 3.5.1 Enfoque Independiente Implementado
        ...
        # Calcular capacidad efectiva = μ + k * σ
        resultados = {}
        for recurso in ["cpu", "ram", "disk"]:
            mu_total = suma_mu[recurso]
            sigma_total = math.sqrt(suma_sigma2[recurso])
            capacidad_efectiva = mu_total + k * sigma_total
    ...
    def cabe_en_servidor(self, servidor: Dict[str, float], usado: Dict[str, float], k: float = 2.0) -> Tuple[bool, Optional[str], Dict[str, float]]: #5.2
        """
        ...
        # 1. Obtener capacidad efectiva (μ + kσ)
        capacidad_efectiva = self.calcular_capacidad_efectica_slice(k)
        Logger.info(f"capacidad_efectiva del slice: {capacidad_efectiva}")
        ...
        for r in ["vcpu", "ram", "disk"]:
            ...
            limite = total * lambdas[r]
            if r == "vcpu":
                demanda = usado_actual + capacidad_efectiva["cpu"]
            else:
                demanda = usado_actual + capacidad_efectiva[r]
    ...
    def calcular_congestion_y_Q(self, servidor: Dict[str, float], usado: Dict[str, float], k: float = 2.0) -> Tuple[float, float]: # 6.2.1,  6.3.2, 6.4.1 
        """
        Calcula:
    ...
    def calcular_score_ponderado(self, servidor: Dict[str, float], usado: Dict[str, float],
                              phi: float = 1.0, num_vms_mismo_sitio: int = 0,
                              k: float = 2.0) -> float:
        """
        ...
        - k: factor de confianza estadística (default 2.0)

        Retorna:
        - score final ponderado (float)
        """
        
        # 1. Verificar si cabe (restricción SLA y seguridad)
        cabe, _, margenes = self.cabe_en_servidor(servidor, usado, k)
        valida_95_seguridad = self.validacion_seguridad_cpu(servidor, usado, k)
        if not cabe:
            Logger.error("No cabe")
            return 0.0
        else:
            Logger.info(f"Cabe slice en servidor y estos son los márgenes: {margenes}")
        """elif not valida_95_seguridad:
            Logger.error("Sobrepasa el 95% de capacidad en el CPU física")
            return 0.0"""
        # 2. Calcular congestión ponderada y Q (usa función ya implementada)
        rho_weighted, Q = self.calcular_congestion_y_Q(servidor, usado, k)
        ...
        f_queue = 1 - Q
        f_perf = phi
        f_cons = 1 + min(num_vms_mismo_sitio, 10) / 20.0
        Logger.info(f"Factor de consolidación: {f_cons}")
        # 4. Score ponderado final
        score = (0.4 * f_cong + 0.3 * f_queue + 0.3 * f_perf) * f_cons
        return round(score, 4)
    
    def seleccionar_mejor_servidor(self, servidores: List[PhysicalServer], k: float = 2.0) -> Tuple[Optional[PhysicalServer], float]:
        ...
        Retorna:
        - (mejor_servidor_dict, score) o (None, 0.0) si ningún servidor cumple
        """
        mejor_score = 0.0
        mejor_servidor = None
